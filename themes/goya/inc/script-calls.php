...
---------------------------------------------------------- */
	...
	define( 'GOYA_SCRIPT_DEBUG', FALSE );
	...
	define( 'GOYA_ASSET_ET_ICON', GOYA_THEME_URI . '/assets/icons/theme-icons/fonts' );
	define( 'GOYA_ASSET_FONT', GOYA_THEME_URI . '/assets/fonts/' );
	...
	function goya_styles() {
		...
		// Default font: Jost
		if (goya_has_default_font()) {
			wp_enqueue_style( 'goya-fonts', GOYA_ASSET_CSS . '/fonts.css', array(), GOYA_THEME_VERSION, 'all' );
		}
    ...
    if ( get_theme_mod('second_font_source', '1') === '2' && get_theme_mod('second_font_typekit_kit_id', '') != '' ) {
		...
		if( goya_wc_active() ) {
			...
			if ( function_exists( 'yith_wishlist_install' ) ){
				wp_enqueue_style( 'goya-wishlist', GOYA_ASSET_CSS . '/wishlist.css', array(), GOYA_THEME_VERSION, 'all' );
			}
...
---------------------------------------------------------- */
	...
	function goya_scripts() {
		...
		if (!is_admin()) {
			...
			// Lazy loading
			wp_enqueue_script( 'lazy-sizes',  GOYA_ASSET_JS . '/vendor/lazysizes.min.js', array( 'jquery' ), '4.1.0', TRUE);
			...
			// Autocomplete
			wp_enqueue_script( 'autocomplete',  GOYA_ASSET_JS . '/vendor/jquery.autocomplete.js', array( 'jquery' ), '1.4.1', TRUE);
			// Magnific Popup
			wp_enqueue_script( 'magnific-popup',  GOYA_ASSET_JS . '/vendor/jquery.magnific-popup.min.js', array( 'jquery' ), '3.0.1', TRUE);
			// Scrollbars
			wp_enqueue_script( 'perfect-scrollbar',  GOYA_ASSET_JS . '/vendor/perfect-scrollbar.jquery.min.js', array( 'jquery' ), '0.7.1', TRUE);
			//Sticky elements
			...
			// Mobile menu
			wp_enqueue_script( 'sliding-menu',  GOYA_ASSET_JS . '/vendor/sliding-menu.min.js', array( 'jquery' ), '0.2.0', TRUE);
			// Manage cookies
			wp_enqueue_script( 'cookie',  GOYA_ASSET_JS . '/vendor/jquery.cookie.js', array( 'jquery' ), '1.4.1');
			...
			if( goya_wc_active() ) { // WooCommerce
				...
				// Image zoom
				wp_enqueue_script('easyzoom', GOYA_ASSET_JS . '/vendor/easyzoom.min.js', array('jquery'), '2.4.0', TRUE);
				// Select2
				wp_enqueue_script( 'select2',  GOYA_ASSET_JS . '/vendor/select2.js', array( 'jquery' ), '3.5.1', TRUE);
			} else {
				// Core App script
			...
			wp_localize_script( 'goya-app', 'theme_vars', array( 
				...
				'l10n' => array (
					'loadmore' => esc_html__("Load More", 'goya'),
					'loading' => esc_html__("Loading ...", 'goya'),
					'back' => esc_html__("Back", 'goya'),
					'nomore' => esc_html__("All Posts Loaded", 'goya'),
					'nomore_products' => esc_html__("All Products Loaded", 'goya'),
					'no_results_found' => esc_html__("No results found.", 'goya'),
				...
				'icons' => array(
					...
					'checkmark' => goya_load_template_part('assets/img/svg/check-circle.svg'),
				),
				'settings' => array (
					...
					'ajaxAddToCartSingle' => get_theme_mod( 'product_single_ajax_addtocart', true ),
					...
					'popup_length' => goya_get_cookie_length(),
...
---------------------------------------------------------- */
	...
	function goya_admin_assets( $hook ) {
		...
		if ( in_array( $hook, array( 'post.php', 'post-new.php', 'toplevel_page_revslider' ) ) ) {
		
			// Default font: Jost
			if (goya_has_default_font()) {
				wp_enqueue_style( 'goya-fonts', GOYA_ASSET_CSS . '/fonts.css', false, GOYA_THEME_VERSION, 'all' );
			}
