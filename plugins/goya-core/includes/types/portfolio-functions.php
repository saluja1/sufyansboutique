...
function goya_get_masonry_size($size = '') {
	...
	switch($size) {
		...
		case 'horizontal':
			...
			break;
		case 'vertical':
			$class_image['class'] = 'col-12 col-md-6 col-lg-3 masonry-vertical';
			$class_image['image_size'] = 'large';
			break;
		case 'small':
		default:
			$class_image['class'] = 'col-12 col-md-6 col-lg-3 masonry-small';
...
function goya_portfolio_categories($categories, $id, $portfolio_id_array = false) {
	...
	<nav class="et-portfolio-filter" id="et-filter-<?php echo esc_attr($id); ?>">
		...
		<ul>
			<li class="active"><a class="tab-link"><?php echo apply_filters( 'portfolio_all_translation',  esc_html('All', 'goya' ) ); ?></a></li>
			<?php 
				foreach ($categories as $cat) {
					$term = get_term($cat);
					echo '<li><a class="tab-link" data-filter="cat-' . esc_attr($term->slug) . '">' . esc_html($term->name) . '</a></li>';
				}
			?>
		</ul>
	</nav>
