...
function goya_vc_template_block( $category ) {
	...
	if ( 'et_templates' === $category['category'] ) {
		...
		<div class="et_templates_container vc_column vc_col-sm-12">
			<div class="et_notice">
				<h3><?php esc_html_e('Append template elements to your page', 'goya' ); ?></h3>
				<ul class="vc_description">
					<li><?php esc_html_e('Elements may show placeholders and need further configuration. Fonts and colors are taken for your customizer settings.', 'goya' ); ?></li>
					</ul>
			</div>
			...
			<?php foreach ($category['templates'] as $key => $template) { ?>
				<div class="et_template theme <?php echo strtolower(implode(' ', $template['cat'])); ?>">
					...
					<div class="theme-actions">
		 					<a class="button button-primary et_template_import" data-et-id="<?php echo esc_attr($key); ?>"><?php esc_html_e('Add', 'goya' ); ?></a>
		 			</div>
				</div>
			<?php } ?>
			</div>
...
function goya_vc_templates( $data ) {
	$et_template_category = array(
		...
		'category_name' => 	esc_html__('Goya', 'goya' ),
		'category_description' => esc_html__('Goya specific demo layouts', 'goya' ),
		'category_weight' => 1,
