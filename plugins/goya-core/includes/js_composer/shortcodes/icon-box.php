	...
	if ( $icon_type === 'icon' ) {
		...
		if ( strlen( $icon ) > 0 ) {
			...
			if($icon_library == 'pixeden') {
				...
				wp_enqueue_style( 'font-awesome', vc_asset_url( 'lib/bower/font-awesome/css/font-all.min.css' ), array(), WPB_VC_VERSION );
			}
		}
		...
		<div class="et-icon-inner">
			...
			<div class="et-feature-content"> 
				...
				<?php if ( strlen( $link ) > 0 ) {
					...
					<a href="<?php echo esc_url( $link['url'] ); ?>" title="<?php echo esc_attr( $link['title'] ); ?>" class="et_btn link"><span class="vc_btn_title"><?php echo esc_attr( $link['title'] ); ?></span></a>
				<?php } ?>
			</div>
		</div>
	</div>
